...
namespace Dino.DremIO.Services
{
    public class DremIOService
    {
        ...
        public DremIOClient Client { get => _dremIOClient; }
    ...
    public class DremIOContext
    {
        ...
        public IAsyncEnumerable<Dictionary<string, object>> QueryWaitAsync(string sqlQuery, int timeOut = 30, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
        ...
        public async IAsyncEnumerable<TModel> QueryWaitAsync<TModel>(string sqlQuery, int timeOut = 30, [EnumeratorCancellation] CancellationToken cancellationToken = default) where TModel : class
        {
            ...
            {
                throw new Exception("Query is fail!");
            }
            var job = _service.CreateJob();
            var res = await job.WaitAsync(jobId, timeOut, cancellationToken: cancellationToken);
    ...
    public class DremIOJob
    {
        ...
        public async Task<JobGetResponse> WaitAsync(string jobId, int timeout = 300, CancellationToken cancellationToken = default) //second
        {
