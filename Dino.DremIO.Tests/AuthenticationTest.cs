...
namespace Dino.DremIO.Tests
{
    public class AuthenticationTest
    {
        ...
        [Fact]
        public async Task QueryAllCancelAsync()
        {
            var provider = HostBuilderTest.Create().Provider;
            var service = provider.GetRequiredService<DremIOService>();
            ...
            var task1 = context.QueryWaitAsync("SELECT * FROM \"youtube-asset-space\".\"asset-label-view\"", cancellationToken: cancelToken.Token).ToListAsync();
            var task = new Task[] { task1.AsTask(),Task.Run(async ()=>
            {
